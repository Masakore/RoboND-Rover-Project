...
def perception_step(Rover):
    ...
    Rover.vision_image[:,:,0] = obs_colorsel * 0
    Rover.vision_image[:,:,1] = rock_colorsel * 100
    Rover.vision_image[:,:,2] = nav_colorsel * 255

    # 5) Convert map image pixel values to rover-centric coords
    obs_xpix, obs_ypix = rover_coords(Rover.vision_image[:,:,0])
    rock_xpix, rock_ypix = rover_coords(Rover.vision_image[:,:,1])
    nav_xpix, nav_ypix = rover_coords(Rover.vision_image[:,:,2])

    # 6) Convert rover-centric pixel values to world coordinates
    scale = 10
    navigable_x_world, navigable_y_world = pix_to_world(nav_xpix, nav_ypix, Rover.pos[0],
    ...
    obstacle_x_world, obstacle_y_world = pix_to_world(obs_xpix, obs_ypix, Rover.pos[0],
                                Rover.pos[1], Rover.yaw,
                                Rover.worldmap.shape[0], scale)
